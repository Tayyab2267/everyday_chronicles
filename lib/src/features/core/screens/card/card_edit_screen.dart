...
class _CardEditScreenState extends State<CardEditScreen> {
  ...
  Future<void> fetchMood() async {
    ...
    final data = await SQLHelper.getItemByDate(widget.cardDate);
    print(" ---> card Date = ${widget.cardDate}");
    if (data.isNotEmpty) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        actions: <Widget>[
          IconButton(
            onPressed: () async {
              ...
              final existingItem = await SQLHelper.getItemByDate(widget.cardDate);
      ...
      body: SingleChildScrollView(
        child: Container(
          margin: const EdgeInsets.only(top: 10, bottom: 50, right: 25, left: 25),
          child: Column(
            children: [
              // 1. Mood
              Material(
