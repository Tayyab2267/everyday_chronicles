...
class AuthenticationRepository extends GetxController {
  ...
  Future<String> _authenticate() async {
    try {
      ...
      bool authenticated = await auth.authenticate(
        localizedReason:
            "Subscribe or you will never find any stack overflow error",
  ...
  Future<void> logout() async {
    ...
    await prefs0.setBool('passcodeEnabled', false);
  ...
  int calculateInitialDelayForWeatherInSeconds(
    ...
    final delays = targetTimes.map((targetTime) {
      if (now.isAfter(targetTime)) {
        final nextDay = now.add(const Duration(days: 1));
        return DateTime(
                nextDay.year, nextDay.month, nextDay.day, targetTime.hour)
            .difference(now)
            .inSeconds;
      } else {
  ...
  Future<void> weatherService() async {
    ...
    await Workmanager().registerPeriodicTask(
      ...
      initialDelay: Duration(
          seconds: calculateInitialDelayForWeatherInSeconds(6, 14, 23)),
